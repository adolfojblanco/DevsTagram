            ...
            @auth
                <nav class="flex gap-2 items-center">
                    ...
                    <a class="font-bold text-sm text-gray-600"
                        href="{{ route('posts.index', Str::of(auth()->user()->name)->slug('-')) }}">Hola,
                        <span class="font-normal">{{ auth()->user()->name }}</span>
                    </a>
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button class="font-bold uppercase text-sm text-gray-600 cursor-pointer" type="submit">Cerrar
                            Sesión</button>
                    </form>
                </nav>
            @endauth
